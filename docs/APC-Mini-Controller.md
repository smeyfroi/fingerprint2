...
**Snapshot cells:** a set cell may carry `SetController::CellKind::Snapshot` instead of the default `Config`. Pressing it applies mod-snapshot slot N (`cell->snapshotSlot`, 0-7) of the **current** config over the running graph — param-only, no config switch, no pager/cursor movement (dispatched via `Synth::applySetCellAction`). The press commits **instantly**: no 400 ms hold, no GUI preview. The pad shows amber while pressed and restores its rest colour on release. Snapshot cells rest at the same authored colour × 0.55 tier as config cells, never memory-dim, and never wear the active white — that colour means "this config is loaded", which a snapshot cell never is. Snapshot recall is deliberately unguarded by hibernation, so these pads work while the synth is hibernated.
